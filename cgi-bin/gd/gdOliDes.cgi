...
my @styles = qw(re ol);
gdheader("Building Block Design (RE Overlap)", "gdOliDes.cgi", \@styles);

if (! $query->param('TARCHNMEL'))
{
...
print <<EOM;
				<div id="notes">
					...
					Your nucleotide sequence will be searched for unique restriction sites, which will be used to divide the sequence into building blocks of approximately 750bp.
					If your sequence is less than 600 bp, there will only be one building block.<br><br>
					Gapped Oligo generation seeks to create building blocks of uniform overlap melting temperature; Ungapped Oligo generation seeks to create building blocks of uniform oligo length.<br><br>
					<em>Please Note:</em><br>
					&nbsp;&nbsp;&bull;Chunk overlap refers to how many bp will be shared in adjacent building blocks.  For instance, with a 20bp chunk overlap, if a 6bp cutter is
					 chosen as a joint it will be padded by 7bp on either side.  Those twenty bp will be in each chunk. <br>
					&nbsp;&nbsp;&bull;The melting temperature you define is an important criteria.  Oligo lengths in practice will decrease or increase to match. <br>
					&nbsp;&nbsp;&bull;Gapped oligos are generated by default.  Uncheck "Generate Gapped Oligos" to make gapless building blocks.<br>
...
##-Figure out what the unique sites are, ignore blunts and one bp overhangs
		...
		my %borders;
		foreach my $enzyme (grep {$$RE_DATA{TYPE}->{$_} !~ /b/ && $$SITE_STATUS{$_} == 1 && ($$RE_DATA{TABLE}->{$_} =~ $IIP || $$RE_DATA{TABLE}->{$_} =~ $IIP2)} keys %{$$RE_DATA{CLEAN}})
		{	
...
####PRE ERROR CHECK - pass only if sequence to be oligo-ed contains a decent number of spaced unique sites
		...
		##-Actually make the chunks, including the left over chunk
			...
			oligocruncher($_, \%pa) foreach (@Chunks);
...
print <<EOM;
				<div id="notes">
					Your sequence has been broken into $y building blocks of approximately 740bp (each).<br>
					<br><em>What am I looking at?</em><br>
					<div>
						<div style="position:absolute;width:50%;">
							<img src="$linkpath/img/olkey1.gif" align="left";>
							<-- building block number<br>
							<-- length of building block<br>
							<-- average Tm of oligo overlaps in building block<br>
							<-- number of oligos in building block<br>
...
EOM
				...
				print "\n", tab(8);
				print "<a href=\"http://rebase.neb.com/rebase/enz/", $chunk->ThreePrimeEnz, ".html\" target=\"blank\">", $chunk->ThreePrimeEnz, "</a>\n" if ($chunk->ThreePrimeEnz);
				print tab(7), "</div>\n";
				print_oligos_aligned($chunk, $pa{gapswit}, 4, 0);
print <<EOM;
							<br><br><br>
							<div id = "olgroup1" style = "background-color:9AB;"width = "1000"><strong>
...
EOM
				for my $j (0..scalar(@oligoarr)-1)
				{
					...
					my $olfme = ($j == 0)					?	" "				:	int(melt($olaparr[$j-1], $pa{melform}) + .5);
					my $oltme = ($j == scalar(@oligoarr)-1)	?	" "				:	int(melt($olaparr[$j], $pa{melform}) + .5);
...
EOM
					my $nextlap = $olaparr[$j]	?	length($olaparr[$j])	:	0;
					$start = $start + length($oligoarr[$j]) - $nextlap;
					push @oligomain, $olseq;
				}
				$start = $start - $chunk->ThreePrimeOlap if ($chunk->ThreePrimeOlap);
print <<EOM;
						</div>
						<br><br><br>
EOM
