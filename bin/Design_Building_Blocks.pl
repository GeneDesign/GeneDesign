...
my $ORIG_SEQUENCE = fasta_parser( $input, 1 );
          ...
          keys %$ORIG_SEQUENCE)
{
  ...
  $pa{per_chn_len}  = $pa{gapswit} == 1  
                      ?  $gapperlen{$pa{tar_oli_len}}      
                      :  $ungapperlen{$pa{tar_oli_len}};
  # these are the defaults, 12 60mers with 20bp overlaps and 20bp gaps, 
  #  nongapped oligos overlap by half the oligo length                      
  $pa{tar_oli_lap}  = $pa{gapswit} == 1  
                      ?  20                    
                      :  .5 * $pa{tar_oli_len};
  $pa{tar_oli_gap}  = $pa{gapswit} == 1  
                      ?  $pa{tar_oli_len}-(2*$pa{tar_oli_lap})  
                      :  0;            # length = 2*(overlap) + gap, nongapped oligos have no gaps.
          ...
          keys %$ORIG_SEQUENCE)
{
  ...
  ##-Figure out what the unique sites are, ignore blunts and one bp overhangs
    ...
    if ($wholelen > ($tar_chn_len+(.2*$tar_chn_len)))
    {
      ...
      {
        my @potents = sort {abs($borders{$a}-$target) <=> abs($borders{$b}-$target)} 
                      grep {$borders{$_} >= 0.5* $tar_chn_len} 
                      keys %borders;
  ...
  ## Load two hashes with the number of times ANxT is seen and where the last one was seen. 
  ## Parse the number hash for the unique ones, push into Data structure and store in array.
